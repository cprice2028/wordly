...
public class LosePanel extends JPanel {
    ...
    public LosePanel(JFrame frame, String answer) {
        this.answer = answer; 
        JPanel pp = new JPanel();
        pp.setLayout(new BorderLayout());
        setLayout(new BorderLayout());
        setBackground(BACKGROUND);
        ImageIcon icon = new ImageIcon("sad.jpeg");
        JLabel imageLabel = new JLabel(icon);
        imageLabel.setHorizontalAlignment(SwingConstants.CENTER);
        add(imageLabel, BorderLayout.CENTER);

        JLabel loseLabel = new JLabel("Sorry, You lose!!", SwingConstants.CENTER);
        loseLabel.setFont(new Font("Clear Sans", Font.BOLD, 36));
        loseLabel.setForeground(TEXT);
        pp.add(loseLabel, BorderLayout.NORTH);
